...
export default function UpcomingPage({ onNavigate }) {
  ...
  return <main className="upcoming-page" ref={pageRef}>
    ...
    <section className="upcoming-page__cta"><p>02 / Make the next thing</p><h2>Have a project<br />waiting to <em>move?</em></h2><div><p>We partner with teams at the beginning — when the right question can change everything that follows.</p><a href="/contact" onClick={(event) => { event.preventDefault(); onNavigate('/contact') }}>Let&apos;s talk <span>→</span></a></div></section>
  </main>
}
